...
```sql
CREATE ROLE ronychang
  WITH LOGIN PASSWORD 'hola123'
       CREATEDB
       INHERIT;
```
...
y luego el DB:
```sql
CREATE DATABASE "Hackaton09DB" OWNER ronychang;
```
...
DB_USERNAME=ronychang       # Username de la DB
DB_PASSWORD=hola123         # Password de la DB
...
curl -X POST http://localhost:3000/api/users \
  ...
  -d '{"firstName":"Rony","lastName":"Chang","email":"rony.ng.2011@gmail.com","password":"hola123"}'
...
curl -s -X POST http://localhost:3000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"roberto.pineda@prueba.com","password":"roberto123"}'
...
curl -X POST http://localhost:3000/api/users \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"firstName":"Brando","lastName":"Lopez","email":"brando@gmail.com","password":"hola123"}'

# 5) Login como student
curl -s -X POST http://localhost:3000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"rony.ng.2011@prueba.com","password":"hola123"}'
